...
def handle_client(client_socket, client_address):
    ...
    try:
        client_socket.sendall(f"Hello {username.capitalize()}, there are currently {len(clients)} (Including you) client(s) online.".encode('utf-8'))
        while True:
            message = client_socket.recv(1024).decode('utf-8')
            if not message: break
            if message.lower() == '!onlineusers': print(f"{username} (Client {list(clients.keys()).index(client_address) + 1}) has requested for currently online users."), client_socket.sendall(f"There are currently {len(clients)-1} other client(s) online.".encode('utf-8'))
                ...
                for recipient_address, recipient_socket in clients.items():
                    if recipient_socket != client_socket and message.lower()!='!onlineusers' and message.lower()!='!exit': recipient_socket.sendall(f'{username} (Client {list(clients.keys()).index(client_address) + 1}): {message}'.encode('utf-8'))
